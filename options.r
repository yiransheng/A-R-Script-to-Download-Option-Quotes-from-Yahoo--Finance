...
###### info.close <- grep("Nasdaq Real Time Price", raw)   
        ...
        if(!all(length(heads) == 16,length(expire) == 2)) stop("Unexpected data format")
        
        info.range <- c(info.open[1], info.close[1])
        ...
        name.pos <- grep("</h2",info.raw)[1]
        span.pos <- grep("</span$",info.raw)
        
        stock.name <- gsub("</h2","",info.raw[name.pos])
        Stock.price <- as.double(gsub("</span","",info.raw[span.pos[3]]))
        Quote.time <- gsub("</span","",info.raw[span.pos[length(span.pos)-2]])
        Quote.date <- paste(Sys.Date(),Quote.time, sep=' ')
        ...
        Quote.date.timeDate <- Sys.timeDate()
...
yahoo.getAllOptions <- function(tickers=c("QQQQ"),File=NULL,imp_vol=TRUE) {
##############################################################################################
...
##############################################################################################
    ...
    for(i in 1:length(tickers)) {
        ...
        short.rate <- get.short.rate(1:1500)  
    ...
    if(imp_vol) df.total$Imp.vol <- imp.vol(df.total)       ## add implied volatilities
